...
export async function PUT(request, { params }) {
  try {
    ...
    if (!cloudinaryConfig.cloudName || !cloudinaryConfig.apiKey || cloudinaryConfig.apiSecret === 'MISSING') {
      return NextResponse.json(
        { 
          ...
          error: 'Please set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in your .env.local file' 
        },
        { status: 500 }
      );
    }
      ...
      try {
        ...
        if (user.profilePhotoPublicId) {
          try {
            ...
            console.warn('Could not delete old profile photo (non-fatal):', deleteError.message);
          }
        }
        ...
        if (!result.url.includes('cloudinary.com') || result.url.includes('localhost')) {
          throw new Error(`Invalid Cloudinary URL returned: ${result.url}`);
        }
        
        profilePhotoUrl = result.url;
        profilePhotoPublicId = result.public_id;
        console.log('✅ Profile photo uploaded successfully to Cloudinary:', result.url);
      } catch (error) {
      ...
      try {
        ...
        if (user.identityDocumentPublicId) {
          try {
            ...
            console.warn('Could not delete old identity document (non-fatal):', deleteError.message);
          }
        }
        ...
        console.log('✅ Identity document uploaded successfully to Cloudinary:', result.url);
      } catch (error) {
    ...
    if (identityDocumentUrl && identityDocumentUrl.includes('cloudinary.com') && !identityDocumentUrl.includes('localhost')) {
      updateData.identityDocumentUrl = identityDocumentUrl;
      updateData.identityDocumentPublicId = identityDocumentPublicId;
      console.log('Updating identityDocumentUrl in database:', identityDocumentUrl);
    }
    
    console.log('Final update data:', {
      ...updateData,
      profilePhotoUrl: updateData.profilePhotoUrl ? 'SET (hidden)' : 'NOT SET',
      identityDocumentUrl: updateData.identityDocumentUrl ? 'SET (hidden)' : 'NOT SET',
    });
    ...
    const updatedUser = await TestUser.findByIdAndUpdate(id, updateData, { new: true }).lean();
    ...
    console.log('profilePhotoUrl:', updatedUser.profilePhotoUrl || 'EMPTY/MISSING');
    console.log('profilePhotoPublicId:', updatedUser.profilePhotoPublicId || 'EMPTY/MISSING');
    console.log('identityDocumentUrl:', updatedUser.identityDocumentUrl || 'EMPTY/MISSING');
    console.log('identityDocumentPublicId:', updatedUser.identityDocumentPublicId || 'EMPTY/MISSING');
    
    // Verify saved data matches what we intended to save
    if (updateData.profilePhotoUrl && !updatedUser.profilePhotoUrl) {
      console.error('❌ WARNING: profilePhotoUrl was in updateData but NOT saved to database!');
    }
    if (updateData.identityDocumentUrl && !updatedUser.identityDocumentUrl) {
      console.error('❌ WARNING: identityDocumentUrl was in updateData but NOT saved to database!');
    }
    console.log('═══════════════════════════════════════════════');
    ...
    if (userResponse.profilePhotoUrl && userResponse.profilePhotoUrl.includes('localhost')) {
      console.error('WARNING: Profile photo URL is localhost, not Cloudinary!');
    }
    if (userResponse.identityDocumentUrl && userResponse.identityDocumentUrl.includes('localhost')) {
      console.error('WARNING: Identity document URL is localhost, not Cloudinary!');
    }
