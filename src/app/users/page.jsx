            ...
            <div className="table-responsive">
              <table className="table table-dark table-hover align-middle">
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u._id || u.id}>
                      <td>{(u._id || u.id).toString().substring(0, 8)}...</td>
                      <td>{u.firstName} {u.lastName}</td>
                      <td>{u.email}</td>
                      <td>{u.country || '-'}</td>
                      <td>{u.gender || '-'}</td>
                      <td><span className="badge bg-secondary">{u.role || 'user'}</span></td>
                      <td className="d-flex gap-2">
                        <Link className="btn btn-sm btn-outline-info text-light" href={`/users/${u._id || u.id}`}>View</Link>
                        <Link className="btn btn-sm btn-outline-secondary text-light" href={`/users/${u._id || u.id}/edit`}>Edit</Link>
                        <button className="btn btn-sm btn-danger" onClick={() => onDelete(u._id || u.id)}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
